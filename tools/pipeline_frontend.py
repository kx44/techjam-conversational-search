...
TOP_K = ev.TOP_K
FIXED_SHOPPER_TARGET = "B07KCFS4VC"
FIXED_SHOPPER_SAMPLE = {
    "sample_id": "manual_columbia_thistletown",
    "scenario_type": "manual_demo",
    "ground_truth": {"parent_asin": FIXED_SHOPPER_TARGET},
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .hidden-item-panel.found {
      border-color: color-mix(in srgb, var(--accent) 48%, var(--border));
      background: color-mix(in srgb, var(--accent) 9%, var(--surface));
    }

    .hidden-item-panel.missed {
    ...
    @media (max-width: 620px) {
      ...
      .search-button, .new-search-button, .hidden-action { width: 100%; }
      .hidden-target-preview { grid-template-columns: 1fr; }
      .hidden-target-art { width: 100%; max-width: 120px; }
...
<body>
    ...
    <section class="search-area">
      <div class="search-card">
        <form id="searchForm" class="search-form">
          ...
          <button id="newSearchBtn" class="new-search-button" type="button">New Search</button>
        </form>
      </div>
    </section>

    <section id="hiddenItemPanel" class="hidden-item-panel idle">
      <span class="hidden-item-label">Hidden item</span>
      <p id="hiddenItemStatus" class="hidden-item-status">Loading a hidden product...</p>
      <div id="hiddenTargetPreview" class="hidden-target-preview"></div>
      <div id="hiddenControls" class="hidden-controls hidden">
        <button id="startHiddenBtn" class="hidden-action" type="button">Start Hidden Search</button>
        <button id="pickHiddenBtn" class="hidden-action secondary" type="button">Pick Different Item</button>
      </div>
    </section>

    <section id="refinePanel" class="refine-panel hidden">
      <div class="reprompt-box">
        <span id="refineStatus" class="refine-status">Narrow your search</span>
        <p id="repromptText"></p>
        <div id="promptHistory" class="prompt-history hidden">
          <div class="prompt-history-title">Prompt history</div>
  ...
  <script>
    ...
    function setLoading(value) {
      state.loading = value;
      const canSearch = state.roundStarted && !state.done;
      $("searchBtn").disabled = value || !canSearch;
      $("promptInput").disabled = value || !canSearch;
      $("newSearchBtn").disabled = value;
      $("startHiddenBtn").disabled = value || state.roundStarted || state.done || state.hiddenItem?.status !== "preview";
      $("pickHiddenBtn").disabled = value;
      if (value) {
    ...
    function renderProducts() {
      const status = state.searchStatus || "empty";
      const isFallback = status === "fallback";
      $("resultsPanel").className = `results-panel ${status}`;
      $("products").className = "product-grid";
      const hidden = state.hiddenItem || {};
      if (state.products.length && status === "final" && hidden.status === "found") {
        $("resultMeta").textContent = `Hidden item found on turn ${hidden.turn}`;
      } else if (state.products.length && status === "preview") {
        $("resultMeta").textContent = `Still narrowing: previewing ${state.products.length} products`;
      } else if (state.products.length && isFallback) {
        $("resultMeta").textContent = `Fallback: best current ${state.products.length} products`;
      } else if (state.products.length) {
        $("resultMeta").textContent = `Search complete: final ${state.products.length} products`;
      } else {
        $("resultMeta").textContent = "No products yet";
      }
      ...
      $("products").innerHTML = state.products.map((product, index) => `
        <article class="product-card ${product.is_hidden_target ? "target-card" : ""}">
          <div class="product-art">
            ${product.image_url ? `<img class="product-photo" src="${esc(product.image_url)}" alt="">` : `<span>${esc(artLabel(product))}</span>`}
          </div>
          <div class="product-body">
            ${product.is_hidden_target ? `<span class="found-badge">Hidden item</span>` : ""}
    ...
    function renderHiddenItem() {
      const hidden = state.hiddenItem || {status: "idle"};
      const product = hidden.product || null;
      const shouldShowProduct = product && ["preview", "found", "missed"].includes(hidden.status);
      $("hiddenItemPanel").className = `hidden-item-panel ${className(hidden.status || "idle")}`;
      $("hiddenTargetPreview").className = shouldShowProduct
        ? "hidden-target-preview show"
        : "hidden-target-preview";
      if (shouldShowProduct) {
        $("hiddenTargetPreview").innerHTML = `
          <div class="hidden-target-art">
            ${product.image_url ? `<img src="${esc(product.image_url)}" alt="">` : `<span>${esc(artLabel(product))}</span>`}
          </div>
          <div class="hidden-target-copy">
            <p class="hidden-target-title">${esc(product.title)}</p>
            <div class="hidden-target-meta">${esc(product.store || "Marketplace seller")}</div>
            <div class="hidden-target-meta">${esc(product.category || product.department || "Catalog item")}</div>
            <div class="hidden-target-meta">${esc(product.id)} - ${esc(money(product))}</div>
          </div>
        `;
      } else {
        $("hiddenTargetPreview").innerHTML = "";
      }
      $("hiddenControls").className = hidden.status === "preview"
        ? "hidden-controls"
        : "hidden-controls hidden";
      if (hidden.status === "preview" && product) {
        $("hiddenItemStatus").textContent = "Memorise this product, then start the hidden search.";
      } else if (hidden.status === "found" && product) {
        $("hiddenItemStatus").textContent = `Found on turn ${hidden.turn}: rank ${hidden.rank || "?"}`;
      } else if (hidden.status === "missed" && product) {
        $("hiddenItemStatus").textContent = `Not found within ${hidden.turn || 10} turns.`;
      } else if (hidden.status === "active") {
        $("hiddenItemStatus").textContent = "Hidden item active. Search from memory.";
      } else {
        $("hiddenItemStatus").textContent = "Loading a hidden product...";
      }
    }

    function renderRefinement() {
      const hasQuestion = state.reprompt || state.suggestions.length;
      $("refinePanel").className = hasQuestion ? "refine-panel" : "refine-panel hidden";
      const narrowing = state.narrowing || {};
      const currentCount = narrowing.match_count;
      const previousCount = narrowing.previous_match_count;
      if (state.searchStatus === "preview") {
        $("refineStatus").textContent = "Still narrowing";
      } else if (state.searchStatus === "fallback") {
        $("refineStatus").textContent = "Fallback results";
      } else if (state.searchStatus === "final") {
        $("refineStatus").textContent = "Search complete";
      } else if (previousCount && currentCount && currentCount < previousCount) {
        $("refineStatus").textContent = `Narrowed ${previousCount} to ${currentCount}`;
      } else if (currentCount) {
        $("refineStatus").textContent = `${currentCount} choices considered`;
      } else {
        $("refineStatus").textContent = "Narrow your search";
      }
      $("repromptText").textContent = state.reprompt || "";
      const prompts = state.promptHistory || [];
      $("promptHistory").className = prompts.length ? "prompt-history" : "prompt-history hidden";
      $("promptList").innerHTML = prompts.map((item) => `
        <li class="prompt-item">
          <strong>Turn ${esc(item.turn)}</strong>
          ${item.state ? `<span class="prompt-state ${className(item.state)}">${esc(item.state_label)}</span>` : ""}
          <span class="prompt-text">${esc(item.prompt)}</span>
        </li>
      `).join("");
    }
    ...
    function render(payload = null) {
      if (payload) {
        ...
        state.reprompt = payload.reprompt || payload.message || "";
        state.suggestions = payload.suggestions || [];
        state.narrowing = payload.narrowing || null;
        state.searchStatus = payload.search_status || state.narrowing?.status || "preview";
        state.promptHistory = payload.prompt_history || [];
        state.hiddenItem = payload.hidden_item || {status: "active"};
        state.done = Boolean(payload.done);
        state.roundStarted = Boolean(payload.started);
      }
      renderProducts();
      renderRefinement();
      renderHiddenItem();
      ...
      $("promptInput").disabled = state.loading || !canSearch;
      $("newSearchBtn").disabled = state.loading;
      $("startHiddenBtn").disabled = state.loading || state.roundStarted || state.done || state.hiddenItem?.status !== "preview";
      $("pickHiddenBtn").disabled = state.loading;
    }
    ...
    async function submitSearch(message) {
      ...
      if (!state.roundStarted) {
        showToast("Look at the hidden product, then click Start Hidden Search.");
        return;
      }
      if (state.done) {
        showToast("Start a new search to play another hidden-item round.");
        return;
      }
      clearToast();
    ...
    function clearRoundView() {
      ...
      state.promptHistory = [];
      state.hiddenItem = {status: "idle"};
      state.done = false;
      state.roundStarted = false;
      $("promptInput").value = "";
      $("promptInput").placeholder = "silver necklace for a birthday gift under $40";
      render();
    }

    async function newHiddenRound() {
      clearRoundView();
      setLoading(true);
      try {
        const payload = await api("/api/new-hidden", {});
        render(payload);
        $("promptInput").placeholder = "Describe the hidden product from memory";
      } catch (error) {
        showToast(error.message);
      } finally {
        setLoading(false);
    ...
    async function startHiddenRound() {
      if (!state.sessionId) {
        await newHiddenRound();
        return;
      }
      clearToast();
      setLoading(true);
      try {
        const payload = await api("/api/start-hidden", {id: state.sessionId});
        render(payload);
        $("promptInput").value = "";
        $("promptInput").placeholder = "Type your first clue, e.g. hiking shoes under $50";
        $("promptInput").focus();
      } catch (error) {
        showToast(error.message);
      } finally {
    ...
    $("newSearchBtn").addEventListener("click", newHiddenRound);
    $("pickHiddenBtn").addEventListener("click", newHiddenRound);
    $("startHiddenBtn").addEventListener("click", startHiddenRound);
...
def _scorecards() -> dict:
    public_dev = dict(PUBLIC_DEV_SCORECARD)
    synthetic = dict(SYNTHETIC_ROBUSTNESS_SCORECARD)
    robustness_path = ROOT / "artifacts" / "synthetic_robustness_100.json"
    if robustness_path.exists():
        try:
            result = json.loads(robustness_path.read_text(encoding="utf-8"))
            overall = result.get("overall") or {}
            config = result.get("config") or {}
            synthetic.update({
                "technical_score": overall.get("technical_score", synthetic["technical_score"]),
                "hit_rate_at_10": overall.get("hit_rate_at_10", synthetic["hit_rate_at_10"]),
                "mrr": overall.get("mrr", synthetic["mrr"]),
                "mttc": overall.get("mttc", synthetic["mttc"]),
                "note": f"{config.get('products', 100)} catalog products",
            })
...
def _active_filter_labels(constraints: dict) -> list[str]:
    labels: list[str] = []
    if constraints["department"]:
        labels.append(_title_value(constraints["department"]))
    if constraints["product_type"]:
        labels.append(_title_value(constraints["product_type"]))
    if constraints["sleeve"]:
        labels.append(_title_value(constraints["sleeve"]))
    labels.extend(_title_value(color) for color in constraints["colors"])
    labels.extend(_title_value(material) for material in constraints["materials"])
    if constraints["budget"] is not None:
        labels.append(f"Under ${constraints['budget']:g}")
    labels.extend(f"Not {_pretty_value(value)}" for value in constraints["exclude_departments"])
    labels.extend(f"Not {_pretty_value(value)}" for value in constraints["exclude_product_types"])
    labels.extend(f"Not {_pretty_value(value)}" for value in constraints["exclude_sleeves"])
    labels.extend(f"Not {_pretty_value(value)}" for value in constraints["exclude_colors"])
    labels.extend(f"Not {_pretty_value(value)}" for value in constraints["exclude_materials"])
    return labels


def _facet_counts(index: "StorefrontIndex", ranked: list[str], prefix: str) -> list[tuple[str, int]]:
    counts: list[tuple[str, int]] = []
    marker = f"{prefix}:"
    for key, members in index.constraint_sets.items():
        if not key.startswith(marker):
            continue
        value = key.split(":", 1)[1]
        count = sum(1 for pid in ranked if pid in members)
        if count:
            counts.append((value, count))
    return sorted(counts, key=lambda item: (-item[1], item[0]))


def _brand_counts(products: dict[str, dict], ranked: list[str]) -> list[tuple[str, int]]:
    counts: dict[str, int] = {}
    for pid in ranked:
        brand = _short(products.get(pid, {}).get("store") or "", 28)
...
def _compatible_suggestion(kind: str, value: str, constraints: dict) -> bool:
    product_type = constraints.get("product_type")
    if kind == "material" and product_type in {"boots", "shoes", "sneakers", "sandals"}:
        return value not in {"cotton", "polyester", "spandex", "rayon", "silk", "linen", "denim"}
    if kind == "material" and product_type in {"necklace", "earrings", "ring", "bracelet"}:
        return value in {"stainless_steel", "sterling_silver", "gold_plated", "alloy", "leather"}
    return True


def _query_for_suggestion(kind: str, value: str) -> str:
    text = _pretty_value(value)
    if kind == "department" and value == "men":
        return "men's"
    if kind == "department" and value == "women":
        return "women's"
    if kind == "budget":
        return f"under ${text}"
    return text
...
def _suggestion(label: str, query: str, question: str) -> dict:
    return {"label": label, "query": query, "question": question}


def _refinement_suggestions(index: "StorefrontIndex", ranked: list[str], constraints: dict,
                            products: dict[str, dict], limit: int = 6) -> list[dict]:
    suggestions: list[dict] = []

    def add(label: str, query: str, question: str) -> None:
        if any(item["label"] == label for item in suggestions) or len(suggestions) >= limit:
            return
        suggestions.append(_suggestion(label, query, question))

    if not constraints["product_type"]:
        add("Item type", "item type", "What type of item are you looking for?")
    if not constraints["department"]:
        add("Who it is for", "who it is for", "Who is it for?")
    if constraints["product_type"] in {"shirt", "jacket", "dress"} and not constraints["sleeve"]:
        add("Sleeve length", "sleeve length", "What sleeve length do you want?")
    if not constraints["colors"]:
        add("Color", "color", "Do you have a preferred color?")
    if not constraints["materials"]:
        add("Material", "material", "Any preferred material or fabric?")
    add("Brand", "brand", "Do you have a preferred brand?")
    if constraints["budget"] is None and len(suggestions) < limit:
        add("Budget", "budget", "Do you have a budget range?")
    return suggestions


def _reprompt_text(product_count: int, active_filters: list[str], suggestions: list[dict],
                   match_count: int | None, previous_match_count: int | None) -> str:
    completion = (
        "Search complete: the final 10 products are shown below."
        if product_count == TOP_K
        else f"Search complete: the final {product_count} matching products are shown below."
    )
    if previous_match_count and match_count and match_count < previous_match_count:
        lead = f"I narrowed the choices. {completion}"
    elif match_count:
        lead = completion
    else:
        lead = completion
    if suggestions:
        question = str(suggestions[0].get("question") or "").strip()
        if question:
            return f"{lead} {question}"
        return f"{lead} Want to add one more detail?"
    return f"{lead} You can search again or start over with a new request."


def _brand_terms_from_message(index: "StorefrontIndex", message: str) -> list[str]:
    """Detect explicit brand/store names typed by the shopper."""
    message_norm = _norm_match(message)
    terms = _tokens(message)
    if message_norm in index.store_names:
        return terms
    return [
        term for term in terms
        if _norm_match(term) in index.store_names
    ]


UNCERTAIN_PIPELINE_MODES = {"explore", "constrain", "flat", "verify"}
PROMPT_STATE_LABELS = {
    "intent_override": "Intent override",
    "boundary": "Boundary",
}
OVERRIDE_TEXT_RE = re.compile(
    r"\b(?:actually|instead|change my mind|changed my mind|ignore|forget|replace|switch|rather|"
    r"start over|new search|reset|no longer|not anymore)\b",
    re.I,
)
BOUNDARY_TEXT_RE = re.compile(
    r"\b(?:not sure|no preference|don't care|do not care|any is fine|either is fine|"
    r"not quite right|ask me about|nothing specific|no specific|no additional preference)\b",
    re.I,
)


def _pipeline_search_status(turn: int, diagnostics: dict) -> str:
    mode = str(diagnostics.get("mode") or "explore")
    if mode in UNCERTAIN_PIPELINE_MODES:
        return "fallback" if turn >= MAX_TURNS else "preview"
    return "final"


def _hidden_search_status(turn: int, hit: bool) -> str:
    if hit:
        return "final"
    if turn >= MAX_TURNS:
        return "fallback"
...
def _pipeline_reply(status: str, product_count: int, question: str) -> str:
    if status == "preview":
        return f"Still narrowing. {question}".strip()
    if status == "fallback":
        return (
            f"Reached the {MAX_TURNS}-turn limit. "
            f"Showing the best current {product_count} products as a fallback."
        )
    return f"Search complete: final {product_count} products."


def _hidden_reply(status: str, product_count: int, question: str, rank: int | None) -> str:
    if status == "final":
        suffix = f" It was rank {rank}." if rank else ""
        return f"Search complete: hidden item found in the final {product_count} products.{suffix}"
    if status == "fallback":
        return (
            f"Hidden item was not found within {MAX_TURNS} turns. "
            f"Showing the best current {product_count} products."
        )
    return f"Still narrowing. {question}".strip()


def _safe_agent_flag(agent: Agent, method_name: str, message: str) -> bool:
    try:
        return bool(getattr(agent, method_name)(message))
    except Exception:
        return False


def _prompt_state(agent: Agent, raw_message: str, diagnostics: dict, reset_requested: bool,
                  previous_ask: str | None) -> str | None:
    if reset_requested or OVERRIDE_TEXT_RE.search(raw_message) or _safe_agent_flag(agent, "_override", raw_message):
        return "intent_override"
    if BOUNDARY_TEXT_RE.search(raw_message) or _safe_agent_flag(agent, "_declined", raw_message):
        return "boundary"
    return None


def _agent_diagnostics(agent: Agent, session_id: str, top_k: int) -> dict:
    state = agent._sessions.get(session_id, {})
    diagnostics = dict(state.get("diagnostics") or {})
    if diagnostics:
        return diagnostics

    plain = agent._query(state.get("plain", {}))
    stems = agent._query(state.get("stems", {}))
    raw_ranking = agent._search("products", plain, agent_mod.RETRIEVE)
    stemmed_ranking = agent._search("products_stem", stems, agent_mod.RETRIEVE)
    dense_ranking = (
        agent._dense_ranking(" ".join(state.get("text", [])), agent_mod.DENSE_LIMIT)
        if agent_mod.USE_DENSE else []
    )
    dense_weight = agent._dense_weight(state, raw_ranking, stemmed_ranking) if dense_ranking else 0.0
    ...
    if agent_mod.USE_EXPANSION:
        rankings.append((
            agent._search(
                "products_stem",
                stems + agent._expand(stemmed_ranking[: agent_mod.FEEDBACK_DOCS], stems),
                agent_mod.RETRIEVE,
            ),
            1.0,
        ))
    fused = agent._fuse([ranking for ranking in rankings if ranking[0]], max(top_k, agent_mod.RERANK_DEPTH))
    diagnostics = agent._diagnose(state, rankings, fused)
...
class StorefrontIndex:
    ...
    def __init__(self, products: dict[str, dict]) -> None:
        ...
        self.store_postings: dict[str, set[str]] = {}
        ...
        for pid, product in products.items():
            ...
            for token in _tokens(store):
                self.store_postings.setdefault(token, set()).add(pid)
            if store_norm:
                self.store_names.add(store_norm)

    def search(self, query: str, top_k: int = TOP_K, constraints: dict | None = None,
               fresh_query: str = "", brand_terms: list[str] | None = None) -> list[str]:
        query = re.sub(r"\s+", " ", query).strip()
        exact_id = query.upper()
        if exact_id in self.products:
            result = [exact_id] + [pid for pid in self._popular(top_k) if pid != exact_id][: top_k - 1]
            self.last_meta = {
                "mode": "exact_asin",
                "allowed_pool": 1,
                "scored_count": 1,
                "returned_count": len(result),
                "candidate_head": result,
            }
            return result
        ...
        brand_terms = [
            term for term in set(brand_terms or [])
            if len(term) >= 3 and 0 < len(self.store_postings.get(term, set())) <= 1000
        ]
        brand_pool: set[str] | None = None
        for term in brand_terms:
            candidates = self.store_postings.get(term, set())
            brand_pool = set(candidates) if brand_pool is None else brand_pool & candidates
        preferred_brand_pool: set[str] | None = None
        if brand_pool:
            narrowed = brand_pool if allowed_pool is None else allowed_pool & brand_pool
            if len(narrowed) >= top_k:
                allowed_pool = narrowed
                preferred_brand_pool = narrowed
            elif narrowed:
                preferred_brand_pool = set(narrowed)
            else:
                preferred_brand_pool = set(brand_pool)
        pool_size = len(allowed_pool) if allowed_pool is not None else len(self.docs)
        self.last_meta = {
            "mode": "ranked",
            "allowed_pool": pool_size,
            "scored_count": 0,
            "returned_count": 0,
            "candidate_head": [],
            "brand_terms": brand_terms,
            "preferred_brand_pool": len(preferred_brand_pool or []),
        }
        terms = _tokens(query)
        fresh_terms = _tokens(fresh_query)
        fresh_norm = _norm_match(fresh_query)
        term_dfs = {term: len(self.postings.get(term, [])) for term in set(terms)}
        rare_terms = [
            term for term, df in term_dfs.items()
            if df and len(term) >= 4 and df <= 250
        ]
        if not terms:
            result = self._popular(top_k, allowed_pool)
            self.last_meta.update({
                "mode": "popular",
                "scored_count": pool_size,
                "returned_count": len(result),
                "candidate_head": result,
            })
            return result
        ...
        for term in rare_terms:
            for pid, weight in self.postings.get(term, []):
                if allowed_pool is not None and pid not in allowed_pool:
                    continue
                scores[pid] = scores.get(pid, 0.0) + weight * 18.0
        for term in fresh_terms:
            for pid, weight in self.postings.get(term, []):
                if allowed_pool is not None and pid not in allowed_pool:
                    continue
                scores[pid] = scores.get(pid, 0.0) + weight * 12.0
        if not scores:
            result = self._popular(top_k, allowed_pool)
            self.last_meta.update({
                "mode": "fallback_popular",
                "scored_count": pool_size,
                "returned_count": len(result),
                "candidate_head": result,
            })
        ...
        for pid in list(scores):
            ...
            if rare_terms:
                rare_title_hits = sum(1 for term in rare_terms if term in title)
                rare_store_hits = sum(1 for term in rare_terms if term in store)
                rare_text_hits = sum(1 for term in rare_terms if term in text)
                scores[pid] += rare_title_hits * 45.0
                scores[pid] += rare_store_hits * 160.0
                scores[pid] += rare_text_hits * 10.0
            if brand_terms:
                brand_hits = sum(1 for term in brand_terms if term in store or term in title)
                if brand_hits or (preferred_brand_pool is not None and pid in preferred_brand_pool):
                    scores[pid] += 1500.0 + brand_hits * 360.0
                else:
                    scores[pid] -= 900.0
            if fresh_terms:
                fresh_title_hits = sum(1 for term in fresh_terms if term in title)
                fresh_store_hits = sum(1 for term in fresh_terms if term in store)
                fresh_text_hits = sum(1 for term in fresh_terms if term in text)
                scores[pid] += fresh_title_hits * 80.0
                scores[pid] += fresh_store_hits * 140.0
                scores[pid] += fresh_text_hits * 18.0
                if fresh_norm:
                    if fresh_norm == store_norm or fresh_norm in store_norm:
                        scores[pid] += 320.0
                    if fresh_norm == title_norm:
                        scores[pid] += 420.0
                    elif fresh_norm in title_norm:
                        scores[pid] += 240.0
                if all(term in title or term in store for term in fresh_terms):
                    scores[pid] += 160.0
        ...
        head = heapq.nlargest(search_depth, scores, key=lambda pid: (scores[pid], pid))
        result = head[:top_k]
        self.last_meta.update({
...
class ShoppingSession:
    def __init__(self, agent_factory, catalog_ids: set[str], products: dict[str, dict],
        ...
        self.previous_match_count: int | None = None
        self.sample: dict = {}
        self.target = ""
        self.started = False
        self.done = False
    ...
    def reset(self) -> None:
        if FIXED_SHOPPER_TARGET not in self.products:
            raise ValueError(f"Fixed hidden product is missing from the catalog: {FIXED_SHOPPER_TARGET}")
        self.agent_session_id = f"shop_agent_{uuid.uuid4().hex}"
        self.agent = None
        self.turn = 0
        self.history = []
        self.prompt_history = []
        self.previous_match_count = None
        self.target = FIXED_SHOPPER_TARGET
        self.sample = next(
            (
                sample for sample in self.samples
                if str(sample.get("ground_truth", {}).get("parent_asin")) == self.target
            ),
            FIXED_SHOPPER_SAMPLE,
        )
        self.started = False
        ...
        self.hit = False
        self.best_rank = None
        self.first_hit_turn = None

    def _ensure_agent(self) -> Agent:
        if self.agent is None:
            self.agent = self.agent_factory()
            self.agent.reset(self.agent_session_id, {})
        return self.agent

    def start(self) -> dict:
        if self.done:
            raise ValueError("This hidden-item round is complete. Start a new search to play again.")
        self.started = True
        return self.payload()

    def _hidden_item(self) -> dict:
        if self.hit:
            status = "found"
        elif self.done:
            status = "missed"
        ...
        payload = {
            "status": status,
            "turn": self.first_hit_turn if self.hit else self.turn,
            "rank": self.best_rank,
        }
        if not self.started or self.done:
            payload["product"] = _product_payload(self.target, self.products.get(self.target, {}))
        return payload

    def payload(self, message: str = "", products: list[dict] | None = None,
                search_status: str = "empty", narrowing: dict | None = None) -> dict:
        return {
            "id": self.id,
            "turn": self.turn,
            "message": message,
            ...
            "narrowing": narrowing or {
                "turn": self.turn,
                "match_count": None,
                "previous_match_count": self.previous_match_count,
                "returned_count": len(products or []),
    ...
    def submit(self, message: str) -> dict:
        ...
        raw_message = message
        reset_requested = bool(re.search(r"\b(start over|new search|reset)\b", message, re.I))
        if reset_requested:
            self.reset()
            return self.payload()
        if not self.started:
            raise ValueError("Look at the hidden product, then click Start Hidden Search.")
        if self.done:
            raise ValueError("This hidden-item round is complete. Start a new search to play again.")

        self.turn += 1
        agent = self._ensure_agent()
        previous_agent_state = agent._sessions.get(self.agent_session_id, {})
        previous_ask = previous_agent_state.get("last_ask")
        response = agent.respond(self.agent_session_id, message, self.turn, TOP_K)
        ...
        hit_rank = ranked.index(self.target) + 1 if self.target in ranked else None
        ...
        for product in products:
            product["is_hidden_target"] = self.done and product["id"] == self.target
        diagnostics = _agent_diagnostics(agent, self.agent_session_id, TOP_K)
        match_count = diagnostics.get("candidate_count")
        previous_match_count = self.previous_match_count
        status = _hidden_search_status(self.turn, self.hit)
        question = str(response.get("message") or "").strip()
        reply = _hidden_reply(status, len(products), question, self.best_rank)
        prompt_state = _prompt_state(agent, raw_message, diagnostics, reset_requested, previous_ask)
        prompt_record = {
            "turn": self.turn,
            "prompt": raw_message,
        }
        if prompt_state:
            prompt_record["state"] = prompt_state
            prompt_record["state_label"] = PROMPT_STATE_LABELS[prompt_state]
        self.prompt_history.append(prompt_record)
        self.previous_match_count = match_count if isinstance(match_count, int) else None
        ...
        narrowing = {
            ...
            "over_general": diagnostics.get("over_general"),
            "flat_pool": diagnostics.get("flat_pool"),
        }
        return self.payload(reply, products, status, narrowing)
...
class PipelineServer:
    ...
    def new_hidden(self) -> dict:
        session = ShoppingSession(self.get_agent, self.catalog_ids, self.products, self.samples)
        self.shopping_sessions[session.id] = session
        return session.payload()

    def start_hidden(self, sid: str) -> dict:
        session = self.shopping_sessions.get(sid)
        if session is None:
            raise ValueError("Hidden-item session expired. Pick a new hidden item.")
    ...
    def do_POST(self) -> None:
        try:
            ...
            if parsed.path == "/api/start":
                ...
                return
            if parsed.path == "/api/new-hidden":
                self._json(200, SERVER.new_hidden())
                return
            if parsed.path == "/api/start-hidden":
                self._json(200, SERVER.start_hidden(str(payload.get("id") or "")))
                return
